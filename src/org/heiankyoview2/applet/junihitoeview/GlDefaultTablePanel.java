...
public class GlDefaultTablePanel extends JDialog implements TablePanel {
	 ...
	 * @param tree Tree 
	 */
	public GlDefaultTablePanel(Tree tree) {
		// super class init
		super();
		if (tree == null) return;
		tg = tree.table;
		...
		//
		Panel p = new Panel();
		p.setLayout(new GridLayout((numTable + 4), 1));
		...
		Label lodtitle = new Label("LOD");
		lodSlider = new JSlider(JSlider.HORIZONTAL, 50, 200, 100);
		p.add(lodSlider);
	...
	public void addSliderListener(ChangeListener changeListener) {
		lodSlider.addChangeListener(changeListener);
	}
	...
	class SliderListener implements ChangeListener {
		public void stateChanged(ChangeEvent e) {
			...
			if (sliderChanged == lodSlider) {
				double lodcoef = (double) lodSlider.getValue() / 10000.0;
				canvas.setLodCoefficient(lodcoef);
			}
		}
	}
