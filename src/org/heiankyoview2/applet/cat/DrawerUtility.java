...
import org.heiankyoview2.core.draw.*;
...
//				+ view.getViewShift(2);
	...
	void depthSortNodes(Branch branch) {
		...
		for (i = 1; i <= branch.getNodeList().size(); i++) {
			...
			if(childBranch == null) {
				treeSet.add((Object) node); continue;
			}
			double minsize = 1.0e+30;
			for(int j = 0; j < childBranch.getNodeList().size(); j++) {
				Node cnode = (Node)childBranch.getNodeList().elementAt(j);
				double size = (cnode.getWidth() < cnode.getHeight())
					? cnode.getWidth() : cnode.getHeight();
				if(minsize > size) minsize = size;
			}
			if(minsize > view.getViewScale() * 0.5)	
