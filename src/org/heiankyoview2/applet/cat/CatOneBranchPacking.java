 ...
 * @author itot
 */
public class CatOneBranchPacking {
	...
	Tree tree;
	Branch branch;
	double gWidth, gHeight, gAspect;
	
	/**
	 * Constructor
	 * @param pg PackingGrid
	 */
	public CatOneBranchPacking(PackingGrid pg) {
		this.pg = pg;
		posX = new double[2];
		posY = new double[2];
	...
	public void placeBranchNodes(Tree tree, Branch branch) {
		...
		this.tree = tree;
		this.branch = branch;
		gWidth  = branch.getParentNode().getWidth();
		gHeight = branch.getParentNode().getHeight();
		gAspect = gHeight / gWidth;
	...
	int trialPlacement(Node node, double xmm[], double ymm[], boolean oflag) {
		...
		double newWidth = (x2 - x1) * gAspect;
		double newHeight = y2 - y1;
		//double oldWidth = pg.xPositions[pg.xSize] - pg.xPositions[0];
		double oldWidth = (pg.xPositions[pg.xSize] - pg.xPositions[0]) * gAspect;
