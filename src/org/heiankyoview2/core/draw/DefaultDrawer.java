...
public class DefaultDrawer implements Drawer {
	 ...
	 */
	public DefaultDrawer(int width, int height, Canvas c) {
		canvas = c;
		imageSize[0] = width;
		imageSize[1] = height;
	...
	void calcBarSize(Node node) {
		...
		zmax = node.getZ(); // + node.getDepth();
		zmin = node.getZ(); // - node.getDepth();
