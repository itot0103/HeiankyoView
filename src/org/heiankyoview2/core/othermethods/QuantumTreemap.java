...
public class QuantumTreemap {
	...
	public Tree placeNodesAllBranch(Tree tree, Tree templateTree,
		...
		if (qMethod == 1)
			System.out.println("  *** Quantum Strip Treemap");
		if (qMethod == 2)
			System.out.println("  *** Quantum Squarified Treemap");
	...
	void placeBranchNodesGrid(Branch branch) {
		int numNode = branch.getNodeList().size();
		Node pnode = branch.getParentNode();
		double branchWidth = pnode.getWidth() * 2.0;
		double branchHeight = pnode.getHeight() * 2.0;
		double branchLeft = pnode.getX() - pnode.getWidth();
		double branchBottom = pnode.getY() - pnode.getHeight();
		int i, j, maxI = 0, maxJ = 0;
		double maxW = 0.0, maxH = 0.0, maxSize = 0.0;
		...
		for (i = 1; i <= numNode; i++) {
			...
			if (numNode % i > 0)
				j++;
			double w = branchWidth / (double) j;
			double h = branchHeight / (double) i;
			double size = (w < h) ? w : h;
			if (maxSize < size) {
				maxSize = size;
				maxI = i;
		...
		//
		// for each node: calculate the positions of nodes
		//
		i = j = 0;
		for (int k = 0; k < branch.getNodeList().size(); k++) {
			Node node = (Node) branch.getNodeList().elementAt(k);
			node.setSize(maxSize * LEAF_NODE_RATIO, maxSize * LEAF_NODE_RATIO, 1.0);
			node.setX(branchLeft + ((double) j + 0.5) * maxW);
			node.setY(branchBottom + ((double) i + 0.5) * maxH);
			if (++j >= maxJ) {
				j = 0;
				i++;
			}
	...
	void calcStripDivider(Vector stripNodes, int parentSize, double parentArea,
		...
		if (flag == 1) {
			...
			for (int j = 0; j < stripNodes.size(); j++) {
				...
				if (snode.getChildBranch() != null) {
					snode.setSize(snodeLength * NON_LEAF_NODE_RATIO,
							stripThick * NON_LEAF_NODE_RATIO, 1.0);
				} else {
		...
		if (flag == 2) {
			for (int j = 0; j < stripNodes.size(); j++) {
				...
				snode.setZ(0.0);
				if (snode.getChildBranch() != null)
					snode.setSize(stripThick * NON_LEAF_NODE_RATIO,
							snodeLength * NON_LEAF_NODE_RATIO, 1.0);
				else {
					double size = (snodeLength < stripThick) ? snodeLength
							: stripThick;
					size *= LEAF_NODE_RATIO;
	...
	public void placeBranchNodesStrip(Branch branch) {
		...
		if (branch.getNodeList().size() == 1) {
			...
			node.setZ(0.0);
			if (node.getChildBranch() != null) {
				node.setSize(parentWidth * NON_LEAF_NODE_RATIO,
						parentHeight * NON_LEAF_NODE_RATIO, 1.0);
			} else {
