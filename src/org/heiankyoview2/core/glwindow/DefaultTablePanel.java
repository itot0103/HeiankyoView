...
public class DefaultTablePanel  extends JDialog implements TablePanel {
	...
	public JButton okButton, cancelButton, applyButton, orderButton;
	...
	public DefaultTablePanel(Tree tree) {
		...
		//
		Panel p1 = new Panel();
		p1.setLayout(new GridLayout((numTable + 4) / 2, 2));
		...
		//
		Panel p2 = new Panel();
		p2.setLayout(new GridLayout((numTable + 4) / 2, 2));
		...
		//
		Panel p3 = new Panel();
		p3.setLayout(new GridLayout((numTable + 4) / 2, 2));
		...
		Panel p4 = new Panel();
		p4.setLayout(new GridLayout((numTable + 2) / 2, 1));
		...
		Panel p5 = new Panel();
		p5.setLayout(new GridLayout((numTable + 2) / 2, 1));
	...
	class ButtonListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (buttonPushed == orderButton){
				//色のラジオボタンでどれがチェックされているかをとってくる
				int selected = -1;;
				for(int i = 0; i <= numTable; i++){
				...
				if(selected == -1){
					System.out.println("色に割り当てる属性を選択してください");
				}
				else{
					NodeOrder no = new NodeOrder();
					no.order(selected, tree);
					tree.resetPlaceFlag();
